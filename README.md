...
│                                                               │
│   GitHub Actions  ──►  SerpAPI  ──►  CSV files in repo        │
...
> **Why SerpAPI?** Amadeus's free self-service portal is being decommissioned
> in mid-2026 and signups are closed. SerpAPI scrapes Google Flights and
> Google Hotels — the exact same data most travellers compare against — and
> their free tier (250 searches/month) covers ~8 daily searches indefinitely.
...
2. A free **SerpAPI** account (3 minutes to sign up — no credit card)
3. About **15 minutes** for first-time setup. After that, zero maintenance.
...
1. Go to <https://serpapi.com/users/sign_up> and create an account (you can
   sign up with Google, GitHub, or email).
2. Verify your email if prompted.
3. After login, you land on the **Dashboard**. Look for **Your Private API Key**
   near the top — that's a long string of letters/numbers.
4. Click the copy icon next to it. Keep this tab open; you'll paste this into
   GitHub in step 3.

You're on the **Free plan**: 250 searches/month, no credit card required, no
auto-upgrade. If you ever exceed it, requests just start failing for the rest
of the month — you won't be charged anything.
...
### Step 3 — Add your SerpAPI key as a secret
...
The secret is encrypted; even you won't be able to read it back.
...
**Watch your quota.** Each daily flight or hotel block uses 1 SerpAPI credit
per day. The free tier gives 250/month, so 8 total blocks ≈ 240/month is the
safe ceiling. Tracking more? Either upgrade SerpAPI (paid plans start at $25/mo
for 1,000 searches), or run the collector less often (every 2 days) by editing
the cron expression.
...
- **GitHub Actions:** free on public repos — unlimited minutes for scheduled
  jobs like this.
- **GitHub Pages:** free for public repos.
- **SerpAPI Free plan:** $0 forever, 250 searches/month, no credit card needed.
- **GitHub Issues:** free.
...
**Workflow run failed with "SERPAPI_KEY must be set":** the secret wasn't added
correctly. Re-do step 3 and check the name matches exactly: `SERPAPI_KEY`.

**Workflow run failed with `429 too many requests`:** you hit the SerpAPI free
quota. Either wait until next month, reduce the number of searches in
`searches.yaml`, or upgrade your SerpAPI plan.

**No data appearing in the dashboard:** check the **Actions** tab → most recent
run → expand "Run collector" step. Errors show up there.

**Hotel results look generic / not what I expected:** the `query` field is a
plain Google Hotels search string. Make it more specific:
- `"hotels in Goa, India"` — broad
- `"hotels in Anjuna, Goa"` — narrower neighbourhood
- `"Taj Resort & Convention Centre Goa"` — a single named property

**Dashboard shows old / stale data:** GitHub Pages can take 1-2 minutes to
update after a workflow run. Hard-refresh the page (Cmd+Shift+R / Ctrl+F5).

**Want to track *current* prices, not future trip dates:** the dates in
`searches.yaml` must be in the future. Pick a fixed date a few months out and
