"""
Daily collector entry point.

Reads searches.yaml, calls Amadeus for each, appends results to CSVs,
fires alerts for price drops, and (re)generates the dashboard HTML.

Run locally:
    AMADEUS_API_KEY=... AMADEUS_API_SECRET=... python -m collector.collect

In GitHub Actions, env vars come from repo secrets.
"""

from __future__ import annotations

import logging
import os
import sys
from datetime import date, datetime

import yaml

from collector import alerts, dashboard, storage
from collector.amadeus_client import AmadeusClient

logging.basicConfig(
    level=os.environ.get("LOG_LEVEL", "INFO"),
    format="%(asctime)s [%(levelname)s] %(name)s: %(message)s",
)
log = logging.getLogger("collect")

ROOT = os.path.dirname(os.path.dirname(__file__))
SEARCHES_PATH = os.path.join(ROOT, "searches.yaml")


def load_searches() -> dict:
    with open(SEARCHES_PATH, encoding="utf-8") as f:
        return yaml.safe_load(f) or {}


def is_future(d: str) -> bool:
    try:
        return datetime.strptime(d, "%Y-%m-%d").date() >= date.today()
    except ValueError:
        return False


def collect_flights(client: AmadeusClient, searches: list[dict]) -> None:
    if not searches:
        return
    existing = storage.read_flights()
    prev_min_by_search: dict[str, float] = {}
    for row in existing:
        sid = row["search_id"]
        try:
            p = float(row["price"])
        except (KeyError, ValueError):
            continue
        if sid not in prev_min_by_search or p < prev_min_by_search[sid]:
            prev_min_by_search[sid] = p

    ts = storage.now_iso()

    for s in searches:
        sid = s["id"]
        if not is_future(s["departure_date"]):
            log.warning("Skipping %s — departure date in past: %s", sid, s["departure_date"])
            continue
        if s.get("return_date") and not is_future(s["return_date"]):
            log.warning("Skipping %s — return date in past: %s", sid, s["return_date"])
            continue

        log.info("Querying flight: %s (%s → %s)", sid, s["origin"], s["destination"])
        offers = client.search_flights(
            origin=s["origin"],
            destination=s["destination"],
            departure_date=s["departure_date"],
            return_date=s.get("return_date"),
            adults=s.get("adults", 1),
            travel_class=s.get("travel_class", "ECONOMY"),
            currency=s.get("currency", "INR"),
            max_results=s.get("max_results", 5),
        )
        if not offers:
            log.warning("  no offers returned")
            continue

        rows = []
        for rank, o in enumerate(offers, start=1):
            rows.append({
                "timestamp": ts,
                "search_id": sid,
                "nickname": s.get("nickname", sid),
                "origin": s["origin"],
                "destination": s["destination"],
                "departure_date": s["departure_date"],
                "return_date": s.get("return_date", ""),
                "rank": rank,
                "price": f"{o.price:.2f}",
                "currency": o.currency,
                "airline": o.airline,
                "stops": o.stops,
                "duration_minutes": o.duration_minutes,
                "departure_time": o.departure_time,
                "arrival_time": o.arrival_time,
            })
        storage.append_flight_rows(rows)
        log.info("  saved %d offers (cheapest: %.2f %s)", len(rows), offers[0].price, offers[0].currency)

        # Alert check
        cheapest = offers[0].price
        prev_min = prev_min_by_search.get(sid)
        threshold = s.get("alert_below")
        # Fire only when: under threshold AND (no prior min OR new low)
        if threshold and cheapest < threshold and (prev_min is None or cheapest < prev_min):
            alerts.flight_alert(s, cheapest, prev_min)


def collect_hotels(client: AmadeusClient, searches: list[dict]) -> None:
    if not searches:
        return
    existing = storage.read_hotels()
    prev_min_by_search: dict[str, float] = {}
    for row in existing:
        sid = row["search_id"]
        try:
            p = float(row["price"])
        except (KeyError, ValueError):
            continue
        if sid not in prev_min_by_search or p < prev_min_by_search[sid]:
            prev_min_by_search[sid] = p

    ts = storage.now_iso()

    for s in searches:
        sid = s["id"]
        if not is_future(s["check_in"]):
            log.warning("Skipping %s — check-in in past: %s", sid, s["check_in"])
            continue

        log.info("Querying hotels: %s (city %s)", sid, s["city_code"])
        hotel_ids = client.list_hotel_ids(s["city_code"], s.get("max_hotels", 10))
        if not hotel_ids:
            log.warning("  no hotels found in city %s", s["city_code"])
            continue

        offers = client.search_hotel_offers(
            hotel_ids=hotel_ids,
            check_in=s["check_in"],
            check_out=s["check_out"],
            adults=s.get("adults", 2),
            rooms=s.get("rooms", 1),
            currency=s.get("currency", "INR"),
        )
        if not offers:
            log.warning("  no hotel offers returned")
            continue

        # Sort by price ascending
        offers.sort(key=lambda o: o.total_price)

        rows = []
        for rank, o in enumerate(offers, start=1):
            rows.append({
                "timestamp": ts,
                "search_id": sid,
                "nickname": s.get("nickname", sid),
                "city_code": s["city_code"],
                "check_in": s["check_in"],
                "check_out": s["check_out"],
                "rank": rank,
                "hotel_id": o.hotel_id,
                "hotel_name": o.hotel_name,
                "price": f"{o.total_price:.2f}",
                "currency": o.currency,
                "room_type": o.room_type or "",
            })
        storage.append_hotel_rows(rows)
        cheapest = offers[0]
        log.info("  saved %d offers (cheapest: %.2f %s @ %s)",
                 len(rows), cheapest.total_price, cheapest.currency, cheapest.hotel_name)

        prev_min = prev_min_by_search.get(sid)
        threshold = s.get("alert_below")
        if threshold and cheapest.total_price < threshold and (prev_min is None or cheapest.total_price < prev_min):
            alerts.hotel_alert(s, cheapest.total_price, cheapest.hotel_name, prev_min)


def main() -> int:
    log.info("Travel price tracker starting…")
    cfg = load_searches()
    flights = cfg.get("flights") or []
    hotels = cfg.get("hotels") or []
    log.info("Loaded %d flight searches and %d hotel searches",
             len(flights), len(hotels))

    if not flights and not hotels:
        log.warning("No searches configured. Edit searches.yaml.")
        return 0

    try:
        client = AmadeusClient()
    except RuntimeError as e:
        log.error(str(e))
        return 1

    collect_flights(client, flights)
    collect_hotels(client, hotels)

    log.info("Generating dashboard…")
    dashboard.generate()
    log.info("Done.")
    return 0


if __name__ == "__main__":
    sys.exit(main())
